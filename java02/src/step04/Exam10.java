...
public class Exam10 {
  ...
  public static void main(String[] args) {
    ...
    // 주소를 저장할 변수를 초기화시키기 => 모든 비트를 0으로 설정
    // v1 = 0; // 0으로 초기화 시킬 것이지만 직접 0으로 설정해서는 안된다.
    v1 = null;  // 주소 변수(레퍼런스 변수)를 초기화 시키는 특별한 값 null을 사용한다.
    
    // 인스턴스 생성
    // => Score 클래스에 선언된 메모리가 준비된다.
    // => 인스턴스의 모든 변수는 0으로 자동 초기화 된다.
    // => 인스턴스 생성 후 리턴 값은 그 메모리 시작 주소이다.
    // => 이렇게 생성된 인스턴스 메모리를 나중에 사용하려면
    //    인스턴스 주소값을 변수에 보관해야한다. -> 레퍼런스 변수
    new Score();  // 이 인스턴스는 주소를 보관하지 않았기 때문에 나중에 사용할 수 없다.
    v1 = new Score(); // 이 인스턴스의 주소는 v1변수에 보관되어 있다.
    
    // 주소를 이용하여 인스턴스 변수에 값을 보관할 수 있다.
    v1.name = "홍길동";  // v1(레퍼런스 변수)에 보관된 주소로 찾아가서 그 메모리의 name변수에 값을 할당.
    
    Score v2 = null;  //레퍼런스 변수의 초기화는 특별한 값 null사용 쓰레기 주소
