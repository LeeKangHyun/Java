...
public class Test07 {
   ...
   * => @RequestParam의 defaultValue 속성 사
   * 클라이언트에서 값이 넘어 오지 않으면 이 속성의 문자열을 사용한다.
   */
  @RequestMapping("e2")
  public String e2(String name, @RequestParam(defaultValue="0")int age) {
    System.out.println(name);
    System.out.println(age);
    return "/test07/e1.jsp";
  }
   ...
   *       만약 이 속성을 지정하지 않으면, 
   *       메서드 파라미터 이름과 일치하는 요청 파라미터(getParameter("xxx"))를 찾는다.
   *    required 속성 : 요청 파라미터 값의 필수/선택 여부 지정
  ...
  /* 요청 파라미터 값을 Value Object 에 바로 받기 */
  @RequestMapping("e5")
